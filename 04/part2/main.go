...
func main() {
	...
	next:
	for _, passphrase := range strings.Split(puzzleInput, "\n") {
...
func anagrams(a, b map[string]int) bool {
	if len(a) != len(b) { return false }
	for letter, aCount := range a {
		if bCount, ok := b[letter]; !ok || aCount != bCount {
			return false
		}
	}
	return true
...
const puzzleInput = `pphsv ojtou brvhsj cer ntfhlra udeh ccgtyzc zoyzmh jum lugbnk
